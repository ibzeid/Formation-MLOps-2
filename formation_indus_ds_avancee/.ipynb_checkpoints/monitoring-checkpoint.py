...
def monitor(latest_predictions: pd.DataFrame) -> pd.DataFrame:
    ...
    date_col = 'predictions_time'
    pred_col = 'predictions'

    latest_predictions[date_col] = pd.to_datetime(latest_predictions[date_col])

    df_resultat = pd.DataFrame({
        'predictions_time': [latest_predictions[date_col].iloc[0]], 
        'predictions': [latest_predictions[pred_col].mean()]
    })
